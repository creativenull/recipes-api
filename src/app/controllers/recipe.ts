...
import {
  recipeCollection as recipes,
  RecipeSchema,
} from "../../db/schema/recipe.ts";
...
const controller = {
  ...
  async recipe(ctx: { id: string }): Promise<RecipeSchema | undefined> {
    const recipeResult =
      (await recipes.findOne({ _id: toMongoId(ctx.id) })) as RecipeSchema;
    const author =
      (await users.findOne({
        _id: toMongoId(recipeResult.author),
      })) as UserSchema;
    return {
      ...recipeResult,
      author: {
  ...
  async createRecipe(
    ...
    const inserted =
      (await recipes.insertOne({ ...ctx.input })) as RecipeSchema;
    const result =
      (await recipes.findOne({ _id: toMongoId(inserted) })) as RecipeSchema;
    return result;
  },

  async updateRecipe(
    ctx: { id: string; input: Recipe },
  ): Promise<RecipeSchema | undefined> {
